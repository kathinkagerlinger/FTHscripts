...
def fromConfig(image_folder, image_numbers, folder_config, number_config, new_bs=False, old_prop=True, topo_nr=None, helpos=None, auto_factor=False, size=[2052,2046], spe_prefix=None, hdf = True):
    '''
    ...
    if hdf:
        nref, center, bs_diam, prop_dist, phase, roi = fth.read_hdf(folder_config + '%i_config.ini'%number_config)
    else:
...
def save_parameters(holo, center, prop_dist, phase, roi, folder, prefix, key_reco, key_params, image_numbers, bs_diam, topo = None, propagate=False):
    '''
    ...
    dreco = pd.DataFrame(recon[roi[2]:roi[3], roi[0]:roi[1]])
    dreco.to_hdf(folder + prefix + '%04d.hdf'%im, key = key_reco)
    
    fth.save_hdf(image_numbers, center, bs_diam, prop_dist, phase, roi, folder + prefix + '%04d.hdf'%im, key_params, topo = topo)
    return
